...
``` r
x <- corx(mtcars[,1:5], describe = list(Mean = function(x) mean(x),
                                        SD = function(x) sd(x)))
x
#> corx(data = mtcars[, 1:5], describe = list(Mean = function(x) mean(x), 
#>     SD = function(x) sd(x)))
#> 
#> ------------------------------------------------
#>        mpg   cyl  disp    hp  drat   Mean     SD
#> ------------------------------------------------
#> mpg     -  -.85* -.85* -.78*  .68*  20.09   6.03
#> cyl  -.85*    -   .90*  .83* -.70*   6.19   1.79
#> disp -.85*  .90*    -   .79* -.71* 230.72 123.94
#> hp   -.78*  .83*  .79*    -  -.45* 146.69  68.56
#> drat  .68* -.70* -.71* -.45*    -    3.60   0.53
#> ------------------------------------------------
#> Note. * p < 0.05
```
